...
export default function Contact() {
  ...
  return (
    <motion.section
      ...
      className="h-screen min-h-[750px] w-full bg-(--sectionBgB) relative overflow-hidden flex items-center transition-colors duration-500"
    >
      {/* Background Glow */}
      ...
      <div className="layout-spine relative z-10 w-full">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
          ...
          <div className="lg:col-span-5 flex flex-col justify-center">
            ...
            <h2 className="text-5xl lg:text-7xl font-heading uppercase text-(--color-text-primary) mb-8 leading-[0.9]">
              Get In <br /> <span className="text-(--zurich-blue-500)">Touch</span>
            </h2>

            <div className="grid grid-cols-1 gap-4 max-w-sm">
              {[
                { icon: MapPin, label: "HQ Command", val: "Plot 123, Jinja Road", accent: "var(--zurich-blue-500)" },
                { icon: Mail, label: "Secure Server", val: "info@smartfisher.go.ug", accent: "var(--militant-green-500)" },
                { icon: Phone, label: "Voice Hotline", val: "0800 100 200", accent: "var(--zurich-blue-500)" }
              ].map((item, idx) => (
                <div 
                  key={idx} 
                  className="glass-card p-4 flex items-center gap-5 border-(--nav-border) hover:border-(--zurich-blue-500)/40 hover:bg-white/[0.03] hover:-translate-y-1 transition-all duration-300 cursor-default group"
                >
                  ...
                  <div>
                    <p className="text-[10px] uppercase tracking-widest text-(--color-text-muted) font-bold mb-0.5">{item.label}</p>
                    <p className="text-sm text-(--color-text-primary) font-body font-semibold tracking-wide">{item.val}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="lg:col-span-7">
            <div className="glass-card p-8 lg:p-12 shadow-2xl border-(--nav-border) relative">
              <form onSubmit={formik.handleSubmit} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-[11px] font-mono uppercase tracking-widest text-(--color-text-muted) ml-1 font-bold">Identity</label>
                    <input
                      name="name"
                  ...
                  <div className="space-y-2">
                    <label className="text-[11px] font-mono uppercase tracking-widest text-(--color-text-muted) ml-1 font-bold">Email</label>
                    <input
                      name="email"
                ...
                <div className="space-y-2">
                  <label className="text-[11px] font-mono uppercase tracking-widest text-(--color-text-muted) ml-1 font-bold">Subject</label>
                  <input
                    name="subject"
                ...
                <div className="space-y-2">
                  <label className="text-[11px] font-mono uppercase tracking-widest text-(--color-text-muted) ml-1 font-bold">Message Transmission</label>
                  <textarea
                    name="message"
                    rows="4"
                ...
                >
                  Transmit Inquiry <Send size={18} className="group-hover:translate-x-1 transition-transform" />
                </button>
              </form>
            </div>
          </div>
